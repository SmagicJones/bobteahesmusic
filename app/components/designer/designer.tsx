...
export default function DesignerDashboard({ user }: DesignerDashboardProps) {
  ...
  const [projectPrices, setProjectPrices] = useState<Record<string, string>>(
    {},
  );
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      for (const userDoc of usersSnap.docs) {
        ...
        projects.forEach((p) => {
          if (p.project.price) {
            prices[p.project.id] = (p.project.price / 100).toFixed(2);
          }
        });
        setProjectPrices((prev) => ({ ...prev, ...prices }));
  ...
  const sendProjectReply = async (
    ...
    const priceInput = projectPrices[projectId];
    ...
    try {
      ...
      if (file) {
        ...
        // Update project price in Firestore if price is set
                  ...
                  <div className="mt-3 flex items-end gap-3">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <input
                        ...
                        min="0"
                        step="0.01"
                        value={projectPrices[project.id] || ""}
                        onChange={(e) =>
                          setProjectPrices({
                            ...projectPrices,
                            [project.id]: e.target.value,
                        ...
                        className="w-24 border p-2 rounded text-slate-900 bg-white"
                        placeholder="5.00"
                      />
                    </div>
                  </div>
